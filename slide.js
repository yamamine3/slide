...
var $author$project$Slide$init = function (_v0) {
...
var $author$project$Slide$subscriptions = function (model) {
	return $elm$core$Platform$Sub$none;
};
var $author$project$Slide$RandomMove = F2(
	function (a, b) {
		return {$: 'RandomMove', a: a, b: b};
	});
...
var $author$project$Slide$Cell = F3(
	function (x, y, n) {
		return {n: n, x: x, y: y};
	});
...
var $author$project$Slide$randomCell = function (cells) {
	return A2(
		$elm$random$Random$uniform,
		A2(
			$elm$core$Maybe$withDefault,
			A3($author$project$Slide$Cell, 0, 0, 0),
			$elm$core$List$head(cells)),
		A2($elm$core$List$drop, 1, cells));
};
var $author$project$Slide$adjacent = F2(
	function (cell1, cell2) {
		return (_Utils_eq(cell1.x, cell2.x) && _Utils_eq(cell1.y, cell2.y)) ? false : (_Utils_eq(cell1.x, cell2.x) ? true : (_Utils_eq(cell1.y, cell2.y) ? true : false));
	});
...
var $author$project$Slide$slide = F2(
	function (cell, conf) {
		var toBeSwapped = A2(
			$elm$core$List$cons,
			cell,
			A2(
				...
				function (c) {
					return (!c.n) && A2($author$project$Slide$adjacent, c, cell);
				},
				conf));
		var others = A2(
			$elm$core$List$filter,
			function (c) {
				return !A2($elm$core$List$member, c, toBeSwapped);
			},
			conf);
		return ($elm$core$List$length(toBeSwapped) === 2) ? _Utils_ap(
			A2(
				$elm$core$List$map,
				function (c) {
					return (!_Utils_eq(c, cell)) ? _Utils_update(
						c,
						{n: cell.n}) : _Utils_update(
						c,
						{n: 0});
				},
				toBeSwapped),
...
var $author$project$Slide$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'Slide':
				var cell = msg.a;
				return (!cell.n) ? _Utils_Tuple2(model, $elm$core$Platform$Cmd$none) : _Utils_Tuple2(
					_Utils_update(
						model,
						{
							conf: A2($author$project$Slide$slide, cell, model.conf)
						}),
					$elm$core$Platform$Cmd$none);
			case 'RandomMove':
				var n = msg.a;
				var cell = msg.b;
				return (!n) ? _Utils_Tuple2(model, $elm$core$Platform$Cmd$none) : ((!cell.n) ? _Utils_Tuple2(
					model,
					A2(
						$elm$random$Random$generate,
						$author$project$Slide$RandomMove(n - 1),
						$author$project$Slide$randomCell(model.conf))) : _Utils_Tuple2(
					_Utils_update(
						model,
						{
							conf: A2($author$project$Slide$slide, cell, model.conf)
					...
					A2(
						$elm$random$Random$generate,
						$author$project$Slide$RandomMove(n - 1),
						$author$project$Slide$randomCell(model.conf))));
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
					...
					A2(
						$elm$random$Random$generate,
						$author$project$Slide$RandomMove(100),
						$author$project$Slide$randomCell(model.conf)));
		}
	});
...
var $author$project$Slide$Shuffle = {$: 'Shuffle'};
...
var $andrewMacmurray$elm_simple_animation$Simple$Transition$delay = $andrewMacmurray$elm_simple_animation$Internal$Transition$Delay;
...
var $author$project$Slide$buttonView = A2(
	$elm$svg$Svg$g,
	_List_fromArray(
		[
			$elm$svg$Svg$Attributes$transform('translate(250,250)'),
			$elm$svg$Svg$Events$onClick($author$project$Slide$Shuffle),
			$elm$svg$Svg$Attributes$class('gold-box-on-hover')
		]),
	_List_fromArray(
		[
			A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							A3($andrewMacmurray$elm_simple_animation$Simple$Transition$property, 'fill', 100, _List_Nil),
							A2(
							$andrewMacmurray$elm_simple_animation$Simple$Transition$color,
							100,
							_List_fromArray(
								[
									$andrewMacmurray$elm_simple_animation$Simple$Transition$delay(50)
								]))
						]))
				]),
			_List_Nil),
...
var $author$project$Slide$Slide = function (a) {
	return {$: 'Slide', a: a};
};
...
var $author$project$Slide$cellView = function (cell) {
	var fy = cell.y;
	var px = (100 * (1 + (2 * cell.x))) * $elm$core$Basics$cos(fy * ((2 * $elm$core$Basics$pi) / 3));
	var py = (100 * (1 + (2 * cell.x))) * $elm$core$Basics$sin(fy * ((2 * $elm$core$Basics$pi) / 3));
	var xystr = '(' + ($elm$core$String$fromFloat(px) + (',' + ($elm$core$String$fromFloat(py) + ')')));
	return A2(
		$elm$svg$Svg$g,
		_List_fromArray(
			[
				$elm$svg$Svg$Attributes$transform('translate' + xystr),
				$elm$svg$Svg$Events$onClick(
				$author$project$Slide$Slide(cell))
			]),
		(!(!cell.n)) ? _List_fromArray(
			[
				A2(
				$elm$svg$Svg$circle,
				_List_fromArray(
					[
						...
						_Utils_eq(cell.y + (3 * cell.x), cell.n) ? 'red' : 'black'),
				...
				_List_fromArray(
					[
						$elm$svg$Svg$text(
						$elm$core$String$fromInt(cell.n))
					])),
				A2(
				$elm$svg$Svg$circle,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$svg$Svg$text(
						$elm$core$String$fromInt(cell.y + (3 * cell.x)))
					]))
			]) : _List_fromArray(
			[
				A2(
...
var $author$project$Slide$check = function (model) {
	return A2(
		$elm$core$List$all,
		function (cell) {
			return _Utils_eq(cell.n, cell.y + (3 * cell.x));
		},
		model.conf);
};
...
var $author$project$Slide$view = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$svg$Svg$Attributes$viewBox('-400 -400 800 800')
					]),
				_Utils_ap(
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									...
									$elm$core$String$fromFloat(
										100 * $elm$core$Basics$cos((2 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$y1(
									$elm$core$String$fromFloat(
										100 * $elm$core$Basics$sin((2 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$x2(
									$elm$core$String$fromFloat(
										300 * $elm$core$Basics$cos((2 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$y2(
									$elm$core$String$fromFloat(
										300 * $elm$core$Basics$sin((2 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$stroke('black'),
									$elm$svg$Svg$Attributes$strokeWidth('2')
								]),
							_List_Nil),
							...
							_List_fromArray(
								[
									$elm$svg$Svg$Attributes$x1(
									$elm$core$String$fromFloat(
										100 * $elm$core$Basics$cos((4 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$y1(
									$elm$core$String$fromFloat(
										100 * $elm$core$Basics$sin((4 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$x2(
									$elm$core$String$fromFloat(
										300 * $elm$core$Basics$cos((4 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$y2(
									$elm$core$String$fromFloat(
										300 * $elm$core$Basics$sin((4 * $elm$core$Basics$pi) / 3))),
									$elm$svg$Svg$Attributes$stroke('black'),
									$elm$svg$Svg$Attributes$strokeWidth('2')
								]),
							_List_Nil),
							$author$project$Slide$buttonView
						]),
					_Utils_ap(
						A2($elm$core$List$map, $author$project$Slide$cellView, model.conf),
						$author$project$Slide$check(model) ? _List_fromArray(
							[
								A2(
								$elm$svg$Svg$text_,
								_List_fromArray(
...
var $author$project$Slide$main = $elm$browser$Browser$element(
	{init: $author$project$Slide$init, subscriptions: $author$project$Slide$subscriptions, update: $author$project$Slide$update, view: $author$project$Slide$view});
_Platform_export({'Slide':{'init':$author$project$Slide$main(
